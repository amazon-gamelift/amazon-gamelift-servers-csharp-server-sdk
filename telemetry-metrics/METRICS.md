...
Integrate the metrics feature into your C# game server to enable telemetry collection. This involves initializing the metrics system and creating metrics for your application.

### Option 1: Initialize Metrics with Default Configuration

The simple approach is to initialize the metrics system with default configuration parameters by calling `InitMetrics()`.
...
// Initialize metrics with defaults (recommended before InitSDK)
...
// Create metrics
var playerJoinsCounter = metrics
    .NewCounter("player_joins")
    .Build();

var activePlayersGauge = metrics
    .NewGauge("server_players")
    .Build();

// Use metrics
playerJoinsCounter.Increment();
activePlayersGauge.Set(42);
```
...
The following environment variables are used by `InitMetrics()` and can override default values:
...
- **GAMELIFT_MAX_PACKET_SIZE** - Maximum UDP packet size in bytes (default: 512)
