...
namespace Aws.GameLift.Server.Model.Metrics.DerivedMetrics
{
    ...
    /// <summary>
    /// Calculate the sum of a list of samples.
    /// </summary>
    public class Sum : BaseDerivedMetric, IDerivedMetric
    {
        /// <summary>
        ...
        /// <returns>The sum of <paramref name="samples"/>, null if empty.</returns>
        public double? CalculateValue(IList<double> samples)
        {
            if (samples == null || samples.Count == 0)
            {
                return null; // Return null if there are no samples to sum
            }
